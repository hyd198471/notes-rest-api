...
export type StateContainer<K extends string, T> = { [key in K]: T };

/** Extract the state type from an ActionReducer. */
export type ActionReducerState<R extends ActionReducer<unknown>> = R extends ActionReducer<infer T> ? T : never;
 ...
 */
export function reducerReplaceItem<T, I>(items: T[], replacementItem: T, identityFn: UnaryFunction<T, I>): T[] {
